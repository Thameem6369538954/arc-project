...
export default function Header() {
  // Creating an array with alternating Round and Triangle images
  const images = Array.from({ length: 10 }, (_, i) => (i % 2 === 0 ? Round : Triangle));
    ...
    <header className="relative w-full min-h-[80vh]  bg-white overflow-hidden flex items-center justify-center ">
      ...
      {images.map((src, index) => (
        <img 
          ...
          src={src} 
          alt={`Shape ${index + 1}`} 
          className={`absolute w-16 h-16 sm:w-20 sm:h-20 md:w-34 md:h-34 object-cover rounded-full transition-transform duration-500 ease-out hover:scale-110 ${positions[index]}`}
  ...
  <div className="text-center">
    {/* Animated Heading */}
    <h1 className="text-4xl md:text-6xl lg:text-8xl text-white font-[Heading] leading-tight 
                   transition-all duration-500 transform hover:scale-105 hover:text-white">
      We take dreams & turn them into families
    </h1>

    {/* Animated Button */}
    <button className="mt-6 px-6 py-3 bg-pink-400 text-white text-lg font-[MyCustomFont] rounded-lg shadow-md 
                       transition-all duration-500 ease-in-out transform hover:bg-pink-500 
      ...
      <div className="w-full md:w-1/3 md:min-h-auto bg-white p-6 rounded-lg shadow-md font-[MyCustomFont] ">
      <div>
